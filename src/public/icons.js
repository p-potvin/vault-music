/**
 * Catppuccin & Lucide Web SVG Icon Pack for Vault Music
 * Clean vector icons without square backgrounds, with Catppuccin palette accents.
 */
...
const SVG_ICONS = {
    ...
    disc: '<circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/><path d="M12 2a10 10 0 0 0-6.88 2.77"/>',
...
function getSvgIcon(name, extraClass = '', size = 20) {
    const inner = SVG_ICONS[name] || SVG_ICONS.music;
    const isFilled = name === 'heart-filled' || name === 'play';
    const fillAttr = isFilled ? 'currentColor' : 'none';
    return `<svg class="v-icon ${extraClass}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="${fillAttr}" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${inner}</svg>`;
}
